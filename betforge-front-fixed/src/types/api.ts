...
export interface Transaction {
  ...
  processed_at?: string;
  /** Data a partir da qual o depósito pode ser sacado (null = sem carência) */
  withdraw_available_at?: string | null;
  created_at: string;
  updated_at: string;
}

/** Depósito ainda em período de carência */
...
/**
 * Resposta de GET /financial/balance
 * Inclui saldo disponível para saque e depósitos em carência (migration 005).
 */
export interface Balance {
  balance: number;
  bonusBalance: number;
  total: number;
  /** Saldo efetivamente disponível para saque (depósitos com carência vencida + ganhos) */
  withdrawableBalance: number;
  /** Depósitos ainda em carência (para exibir aviso ao usuário) */
