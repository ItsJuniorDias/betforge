 ...
 *
 * Fluxo de saque (processamento manual):
 *  1. Valida saldo, KYC e limites
 *  2. Debita o saldo imediatamente (reserva)
 *  3. Cria transação 'pending'
 *  4. Registra dados do saque no log para processamento manual
 *  5. Admin efetua o PIX manualmente pelo app do Mercado Pago
 *  6. Após confirmar o envio, chamar PATCH /financial/withdraw/:id/confirm
 *     para marcar a transação como 'completed'
 *
 * TODO: substituir por integração automática (Asaas, Pagar.me, etc.)
 *       quando o provider de payout estiver contratado.
 */
...
export const FinancialService = {
  async deposit(dto: DepositDTO) {
    ...
    if (amount < limits.min)
      throw new AppError(
        `Valor mínimo para ${method} é R$ ${limits.min.toFixed(2)}`,
      );
    if (amount > limits.max)
      throw new AppError(
        `Valor máximo para ${method} é R$ ${limits.max.toFixed(2)}`,
      );
    ...
    try {
      ...
      if (method === "pix") {
        ...
        await TransactionRepository.updateExternalId(
          transaction.id,
          String(payment.id),
        );
        paymentData = {
        ...
        if (!cardToken) {
          throw new AppError(
            "Token do cartão obrigatório para pagamento com cartão",
            400,
          );
        }
  ...
  async withdraw(dto: WithdrawDTO) {
    ...
    // ─── Processamento manual de saques ───────────────────────────────────────
    // O Mercado Pago não expõe API pública de payout para terceiros no Brasil.
    // Por enquanto o saque fica em 'pending' e o admin envia o PIX manualmente
    // pelo app do MP. Após confirmar o envio, chamar confirmWithdraw() abaixo
    // para marcar como 'completed'.
    //
    // TODO: substituir pelo provider de payout escolhido (Asaas, Pagar.me, etc.)
    logger.info("Saque solicitado — processamento manual necessário", {
      transactionId: transaction.id,
      userId,
      amount,
      method,
      pixKey: method === "pix" ? pixKey : undefined,
      pixKeyType: method === "pix" ? (pixKeyType ?? "cpf") : undefined,
  ...
  // ─── Confirmação manual de saque (chamada pelo admin) ─────────────────────
  async confirmWithdraw(transactionId: string) {
    ...
    if (tx.type !== "withdraw")
      throw new AppError("Transação não é um saque", 400);
  ...
  // ─── Cancelamento de saque pendente (admin ou o próprio usuário) ───────────
