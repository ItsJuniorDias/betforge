 ...
 */
async function syncIfNeeded(sports: Array<'football' | 'basketball'> = ['football', 'basketball']) {
...
export const GamesController = {
  ...
  async getMarkets(req: Request, res: Response) {
    ...
    if (!markets.length) throw new NotFoundError('Partida não encontrada ou sem mercados');
  ...
  async forceSync(req: Request, res: Response) {
    if (!env.ODDS_API_KEY) {
      return Res.success(res, { message: 'ODDS_API_KEY não configurada, nada a sincronizar.' });
    }
