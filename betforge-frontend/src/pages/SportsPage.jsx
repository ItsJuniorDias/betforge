...
import { sports, liveGames, upcomingGames, leagues } from "../data/mockData";
...
export default function SportsPage() {
  ...
  const sportGames = allGames.filter((g) => g.sport === activeSport);
  const sportLeagues = leagues.filter((l) => l.sport === activeSport);

  const leagueFilteredGames =
    activeLeague === "all"
      ? sportGames
      : sportGames.filter((g) => g.league.toLowerCase().includes(activeLeague));
  ...
  return (
    <div className="p-5 max-w-full mx-auto">
      ...
      {leagueFilteredGames.length > 0 ? (
        <>
          {/* Live */}
          {leagueFilteredGames.filter((g) => g.isLive).length > 0 && (
