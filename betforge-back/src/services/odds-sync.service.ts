...
async function upsertMatch(
  ...
  if (score?.completed) {
    status = "finished";
  } else if (score && score.scores && score.scores.length > 0) {
    status = "live";
  } else {
    status = "scheduled";
  }
...
export const OddsSyncService = {
  ...
  async sync(
    ...
    try {
        ...
        try {
          ...
          for (const event of events) {
            try {
              ...
              logger.warn(
                `[OddsSync] erro ao fazer upsert do evento ${event.id}: ${err}`,
              );
            }
          }

          logger.info(
            `[OddsSync] ${sportKey}: ${events.length} jogos sincronizados`,
          );
        } catch (err) {
      ...
      logger.info(
        `[OddsSync] sync concluído em ${Date.now() - started}ms — ${totalEvents} eventos`,
      );
    } finally {
