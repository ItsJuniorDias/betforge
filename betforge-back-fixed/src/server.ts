...
async function bootstrap() {
  try {
    ...
    // Liquida apostas de partidas finalizadas a cada 5 minutos.
    // O intervalo pode ser ajustado via SETTLEMENT_INTERVAL_MS no .env.
    const settlementInterval = Number(process.env.SETTLEMENT_INTERVAL_MS) || 5 * 60 * 1000;
    const settlementTimer = SettlementService.startScheduler(settlementInterval);
